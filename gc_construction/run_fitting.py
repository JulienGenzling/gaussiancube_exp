...
def run_on_gpu(obj_chunk, gpu_id, args):
    for obj in obj_chunk:
        ...
        command = f"python train.py -s {obj} -m {os.path.join(args.output_path, obj_name)} --dataset_type shapenet --white_background --test_iterations 7000 30000 --port {PORT+gpu_id} --densification_interval 50 --sh_degree 0 --no_tqdm"
